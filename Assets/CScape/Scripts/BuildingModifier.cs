        ...
        public void ModifyBuilding()
        {
            if (gameObject.activeInHierarchy)
            {
                ...
                while (i < vertices.Length)
                {
                    ...
                    {
                        if (useGraffiti)
                            vertexColor[i] = new Color32(vertexColor[i].r, vertexColor[i].g, vertexColor[i].b, 255);
                        else vertexColor[i] = new Color32(vertexColor[i].r, 255, 0, 0);
                    }
                    ...
                    if (autoMap)
                    {
                        if (invNormal.z < 1 + normalThreshold && invNormal.z > 1 - normalThreshold)
                        {
                            uV[i] = new Vector2(vertices[i].x / floorHeight, vertices[i].y / floorHeight);
                        }
                        ...
                        if (normals[i].x < 1 + normalThreshold && normals[i].x > 1 - normalThreshold)
                        {
                            uV[i] = new Vector2(vertices[i].z / floorHeight, vertices[i].y / floorHeight);
                        }
