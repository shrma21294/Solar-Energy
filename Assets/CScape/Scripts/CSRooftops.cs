            ...
            if (gameObject.activeInHierarchy)
            {
                ...
                if (!isPrefabOriginal)
                {
                    ...
                    for (int j = 0; j < surfacedensity; j++)
                    {
                        ...
                        newObject.transform.position = new Vector3(Random.Range(minZ.x, maxX.x), minZ.y, Random.Range(minZ.z, maxZ.z)) - rooftopHolder.transform.position;
                    ...
                    rooftopHolder.transform.localScale = gameObject.transform.localScale;
                    //   if (meshF.sharedMesh.vertices.Length == 0) DestroyImmediate(rooftopHolder);
                }
            }
